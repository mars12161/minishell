...
/*   Updated: 2023/07/12 22:52:29 by yli              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *ft_parse_quote_rules(char *str, t_env **env)
{
	...
	{
		result = ft_substr(str, 1, ft_count_size_lexer(str, signal, 1) - 1);
		if (signal == 34)
			return (ft_parse_dollar_frame(result, *env));
		else
			return (result);
	}
    result = ft_parse_quote_rules_substr(str, ft_count_size_lexer(str, signal, 1), env);
	printf("return 2: result: %s\n", result);
    if (ft_count_size_lexer(str, 34, 0) == ft_count_size_lexer(str, 39, 0) == ft_strlen(str) || str[0] == 39)
        return (result);
    if (str[0] == 34)
